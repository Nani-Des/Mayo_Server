...
/**
 * Kafka consumer for medical record events
 */
@Service
@Slf4j
...
public class MedicalRecordEventConsumer {
    ...
    @KafkaListener(topics = Topics.MEDICAL_RECORD_EVENTS, groupId = "audit-service", concurrency = "#{${audit.kafka.consumer.concurrency:3}}", containerFactory = "auditKafkaListenerContainerFactory")
    @Transactional
    public void consumeMedicalRecordEvent(
        ...
        try {
            log.debug("Received medical record event: key={}, offset={}", key, offset);

            // Parse the medical record event
            ...
            try {
                ...
                log.debug("Indexed medical record event in Elasticsearch: {}", savedEvent.getEventId());
            } catch (Exception e) {
                log.error("Failed to index medical record event in Elasticsearch: {}", savedEvent.getEventId(), e);
            ...
            log.info("Successfully processed medical record event: {}", savedEvent.getEventId());

        } catch (Exception e) {
            log.error("Failed to process medical record event: key={}, error={}", key, e.getMessage(), e);
            // In a production system, you might want to send to a dead letter queue
            // or implement retry logic here
            throw new RuntimeException(e); // Re-throw to trigger retry or dead letter handling
        }
    }
    ...
    private AuditEvent parseEvent(String message, String key) throws Exception {
        ...
        if (jsonNode.has("ipAddress")) {
            try {
                ...
                log.warn("Invalid IP address in medical record event: {}", jsonNode.get("ipAddress").asText());
            }
        }
