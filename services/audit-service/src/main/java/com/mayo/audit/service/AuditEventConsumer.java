...
public class AuditEventConsumer {
    ...
    @KafkaListener(topics = Topics.AUDIT_EVENTS, groupId = "audit-service", concurrency = "#{${audit.kafka.consumer.concurrency:3}}", containerFactory = "auditKafkaListenerContainerFactory")
    @Transactional
    public void consumeAuditEvent(
        ...
        try {
            log.debug("Received audit event: key={}, offset={}", key, offset);

            // Parse the audit event
            AuditEvent auditEvent = parseAuditEvent(message, key);
            ...
            try {
                ...
                log.debug("Indexed audit event in Elasticsearch: {}", savedEvent.getEventId());
            } catch (Exception e) {
                log.error("Failed to index audit event in Elasticsearch: {}", savedEvent.getEventId(), e);
            ...
            log.info("Successfully processed audit event: {}", savedEvent.getEventId());

        } catch (Exception e) {
            log.error("Failed to process audit event: key={}, error={}", key, e.getMessage(), e);
    ...
    private AuditEvent parseAuditEvent(String message, String key) throws Exception {
        ...
        if (jsonNode.has("ipAddress")) {
            try {
                ...
                log.warn("Invalid IP address in audit event: {}", jsonNode.get("ipAddress").asText());
            }
        }
