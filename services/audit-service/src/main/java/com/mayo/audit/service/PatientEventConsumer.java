...
public class PatientEventConsumer {
    ...
    @KafkaListener(topics = Topics.PATIENT_EVENTS, groupId = "audit-service", concurrency = "#{${audit.kafka.consumer.concurrency:3}}", containerFactory = "auditKafkaListenerContainerFactory")
    @Transactional
    public void consumePatientEvent(
        ...
        try {
            log.debug("Received patient event: key={}, offset={}", key, offset);

            // Parse the patient event
            ...
            try {
                ...
                log.debug("Indexed patient event in Elasticsearch: {}", savedEvent.getEventId());
            } catch (Exception e) {
                log.error("Failed to index patient event in Elasticsearch: {}", savedEvent.getEventId(), e);
            ...
            log.info("Successfully processed patient event: {}", savedEvent.getEventId());

        } catch (Exception e) {
            log.error("Failed to process patient event: key={}, error={}", key, e.getMessage(), e);
            // In a production system, you might want to send to a dead letter queue
            // or implement retry logic here
            throw new RuntimeException("Failed to process patient event", e); // Re-throw to trigger retry or dead
                                                                              // letter handling
        }
    }
    ...
    private AuditEvent parseEvent(String message, String key) throws Exception {
        ...
        if (jsonNode.has("ipAddress")) {
            try {
                ...
                log.warn("Invalid IP address in patient event: {}", jsonNode.get("ipAddress").asText());
            }
        }
