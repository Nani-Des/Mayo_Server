...
public class HospitalEventConsumer {
    ...
    public void consumeHospitalEvent(
        ...
        try {
            log.debug("Received hospital event: key={}, topic={}, partition={}, offset={}",
                     key, topic, partition, offset);

            // Parse the hospital event JSON
            ...
            log.info("Successfully processed hospital event for user: {}", userId);

        } catch (Exception e) {
            log.error("Failed to process hospital event: {}", message, e);
